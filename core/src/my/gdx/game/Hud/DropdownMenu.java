...
public class DropdownMenu extends Hud {
    ...
    public DropdownMenu(float xpos, float ypos, Entity ent) {
        ...
        if (ent.inventory != null && ent.getPos().dst(EveOnline2.player.getPos()) <= 100000
                * (EveOnline2.player.getSize() + ent.getSize()) * Entity.METER) {
            addbutton(new DropdownButton(this.x, nextopenspace, "Open Inventory", target) {// define custom method for
                                                                                           // this button
                @Override
        ...
        if (!ent.equals(EveOnline2.player)) {
            addbutton(new DropdownButton(this.x, nextopenspace, "Approach", target) {// define custom method for this
                                                                                     // button
                @Override
                public void interact(float x, float y) {
            ...
            addbutton(new DropdownButton(this.x, nextopenspace, "Warp to", target) {// Warp-To Button
                ...
                public void updateShape() {
                    ...
                    if (isactive && EveOnline2.player.getRotation().cpy().crs(direction).len2() <= 0.99) {
                        if (EveOnline2.player.getPos().dst(t.getPos()) > (t.getSize() + EveOnline2.player.getSize())
                                * 10) {
    ...
    public void dispose() {
        if (buttons != null) {
            for (Button b : buttons) {
                if (!EveOnline2.windows.contains(b))
                    b.dispose();
                else
                    b.moveTo(screenwidth / 2, 160);
            }
            buttons = null;
        }
        this.type = null;
