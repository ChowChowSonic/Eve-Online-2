...
public abstract class Entity implements Serializable {
	...
	*/
	...
	/**
	* Takes a dataOutputStream and writes info about this entity to the stream. Each Entity has a minimum of 56 bytes written, NOT including the Entity's modelname. <p>
	* This method does NOT serialize: The Inventory, the Instance, The Model (although this is not required to be sent), & the Direction. 
	* @param s - The DataOutputStream to write this entity to
	* @throws IOException
	*/
	public void Serialize(DataOutputStream s) throws IOException{
		s.writeLong(this.ID); 
		s.writeInt(this.type.ordinal());
		s.writeFloat(this.pos.x); //Position values
		s.writeFloat(this.pos.y);
		s.writeFloat(this.pos.z);
		s.writeFloat(this.vel.x); //Velocity values
		s.writeFloat(this.vel.y);
		s.writeFloat(this.vel.z);
		s.writeFloat(this.accel.x); //Acceleration values
		s.writeFloat(this.accel.y);
		s.writeFloat(this.accel.z);
		s.writeFloat(this.mass);
		s.writeFloat(this.size);//56 Bytes
		s.writeUTF(this.modelname);
	}
	
	/**
	 * Deserializes a string of data and writes entity info from this data. Starting with the player's position,
	 * velocity & acceleration, this method skips reading the entity's ID & Type. The ID/Type of the entity is needed to 
	 * identify what kind of entity it is, so a call to DataInputStream.readLong() & DataInputStream.readInt() prior to 
	 * calling this method will result in proper entity deserialization. 
	 * @param s
	 * @throws IOException
	 */
	public void Deserialize(DataInputStream s) throws IOException{
		this.pos = new Vector3(s.readFloat(), s.readFloat(), s.readFloat()); //Position values
		this.vel = new Vector3(s.readFloat(), s.readFloat(), s.readFloat()); //Velocity values
		this.accel = new Vector3(s.readFloat(), s.readFloat(), s.readFloat());//Acceleration values
		this.mass=s.readFloat();
		this.size=s.readFloat();//56 Bytes
		this.modelname=s.readUTF();
	}
	...
	public boolean equals(Object e) {
		...
		}else if(e instanceof Long) return e.equals(this.ID); 
		return false; 
	}
